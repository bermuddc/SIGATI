...
try {
    ...
    */
    ...
    /* Contar todos los resultados antes de aplicar la paginación. */
    $sql_conteo = "
        SELECT COUNT(DISTINCT n.id_notebook)
        FROM notebook n
        INNER JOIN estado_notebook e
            ON n.id_estado = e.id_estado
        LEFT JOIN asignacion a
    ...
    <section class="tabla-contenedor">
        ...
        <?php if (count($notebooks) > 0): ?>
            ...
            >
                ...
                <a
                    class="boton boton-secundario <?= $pagina <= 1 ? 'deshabilitado' : ''; ?>"
                    href="<?= e(url_pagina(
                        max(1, $pagina - 1),
                        $busqueda,
                        $piso_valido
                ...
                <a
                    class="boton boton-secundario <?= $pagina >= $total_paginas ? 'deshabilitado' : ''; ?>"
                    href="<?= e(url_pagina(
                        min($total_paginas, $pagina + 1),
                        $busqueda,
                        $piso_valido
                    )); ?>"
                >
